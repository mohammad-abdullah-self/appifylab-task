...
export default function CreateFeedForm() {
  const [state, action, pending] = useActionState(createPostAction, false);
  ...
  return (
    ...
    >
      <input type="file" name="image" ref={fileInputRef} className="d-none" />
      {/* --- VISIBILITY SELECT ADDED HERE --- */}
      ...
      <ValidationError message={state?.errors?.image} />
    </form>
  );
}
