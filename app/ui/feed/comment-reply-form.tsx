...
import { createReplyAction } from "@/actions/post";
...
const CommentReplyForm: FC<Props> = ({ comment, post }) => {
  const [state, action, pending] = useActionState(createReplyAction, false);
  return (
    <>
      <div className="_feed_inner_comment_box">
        <form action={action} className="_feed_inner_comment_box_form">
        ...
        <ValidationError message={state?.errors?.content} />
        <ValidationError message={state?.errors?.postId} />
      </div>
    </>
  );
};
